...
from cocotb.triggers import Edge, Combine, NextTimeStep, First
...
async def watch_counter_val(dut):
    if "GL" not in cocotb.plusargs:
        counter_val_hdl = dut.uut.user_project.openframe_example.counter_timer_0.counter_timer_high_inst.value_cur
    else:
        example_hdl = dut.uut.user_project.openframe_example
        counter_val_hdl = [example_hdl._id(f"\counter_timer_0.counter_timer_low_inst.reg_dat_do[{i}] ", False) for i in range(32)]
    ...
    while True:
        ...
        cocotb.log.info(f"[watch_counter_val] counter_val = {hex(counter_val)}")
